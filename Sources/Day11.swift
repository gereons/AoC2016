...
final class Day11: AdventOfCodeDay {
    let title = "Radioisotope Thermoelectric Generators"
    ...
    struct Grid: Pathfinding {
        func distance(from: Building, to: Building) -> Int {
            1
        }

        func neighbors(of building: Building) -> [Building] {
    ...
    init(input: String) {
        building = Building(input.lines.map { Floor($0) }, elevator: 0)
    }
    ...
    private func solve(_ building: Building) -> Int {
        ...
        let path = pathfinder.shortestPath(from: building, to: target)
